...
<body>
	<div class="container-fluid">
		...
		<form action="<%=basePath%>interact/comment.do?proId=${pro.proId}"
			method="post">
			<input type="hidden" name="proNo" value="${pro.proId }">
			<input type="hidden" name="commentAuthor" value="${name}">
				<label for="comment" class="btn btn-info disabled">评论:</label>
				<textarea class="form-control" placeholder="请文明发言" rows="5"
					id="comment" name="commentContent"></textarea>
			<input type="submit" value="发布">
		</form>
		<a href="<%=basePath%>apply/applyUI.do?proTitle=${pro.proTitle}"
			class="btn btn-lg btn-outline-success">去报名参加该活动</a>
