		...
		<table class="table table-bordered text-center bg-light text-dark">
			<tr>
				<td class="text-left" colspan="5"><a
					href="<%=basePath%>social/addUI.do?" class="btn btn-primary btn-sm">添加</a></td>
			</tr>
			<tr>
				<td>账号登录名</td>
				<td>账号密码</td>
			...
			<c:forEach items="${socials }" var="social">
				<tr>
					<td>${social.socialLogin }</td>
					<td>${social.socialPassword }</td>
					<td>${social.socialDesc }</td>
					<td><a
						href="<%=basePath%>social/delete.do?socialId=${social.socialId }"
						class="btn btn-primary btn-sm alert alert-danger">删除</a> <a
						href="<%=basePath%>social/updateUI.do?socialId=${social.socialId }"
