...
<body>
	<div class="container-fluid">
		...
		<div class="collapse navbar-collapse" id="navbarNav">
			<ul class="navbar-nav">
				<li class="nav-item btn btn-info disabled">资源列表展示</li>
			</ul>
		</div>
		</nav>
		<table class="table table-bordered text-center bg-light text-dark">
			<tr>
				<td colspan="5" class="text-left"><a href="<%=basePath%>resource/addUI.do?"
			...
			<c:forEach items="${resources }" var="resource">
				<tr>
					...
					<td>${resource.resourceState }</td>
					<td><a
						href="<%=basePath%>resource/delete.do?resourceId=${resource.resourceId }"
						class="btn btn-primary btn-sm alert alert-danger">删除</a> <a
						href="<%=basePath%>resource/updateUI.do?resourceId=${resource.resourceId }"
